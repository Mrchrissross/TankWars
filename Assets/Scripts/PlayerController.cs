...
/// <summary>
/// The Player Controller controls the movement, style and shooting of the player tank.
/// </summary>
public class PlayerController : MonoBehaviour
{
    ...
    [Header("Shooting: "), Tooltip("Time between shots in seconds."), VectorLabels("Timer", "Count")]
    ...
    {
        if(shotTimer.y > 0f)
            shotTimer.y -= Time.deltaTime;                                      

        if (Input.GetMouseButtonDown(0) && shotTimer.y < Mathf.Epsilon)         
        {
            GameObject bullet = AssetManager.Instance.SpawnObject("Shoot", firePoint.position, firePoint.rotation);                 
